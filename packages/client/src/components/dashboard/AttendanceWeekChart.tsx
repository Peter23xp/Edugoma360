...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-white border border-neutral-200 rounded-lg shadow-md px-3 py-2 text-xs">
      <p className="font-semibold text-neutral-900">{d.day} {new Date(d.date).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' })}</p>
      <p className="text-neutral-600">{d.present}/{d.total} élèves : <span className="font-bold">{d.rate}%</span></p>
    </div>
  );
};
...
export default function AttendanceWeekChart({ data, isLoading }: Props) {
  ...
  return (
    <div className="bg-white border border-neutral-200 rounded-lg p-4">
      <h3 className="text-sm font-semibold text-neutral-800 mb-3">Présence — 7 derniers jours</h3>
