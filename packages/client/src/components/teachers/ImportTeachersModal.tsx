...
export default function ImportTeachersModal({ isOpen, onClose }: ImportTeachersModalProps) {
    ...
    const handleFileSelect = useCallback(async (selectedFile: File) => {
        ...
        try {
            ...
            const mapped: ParsedRow[] = jsonData.map((row) => ({
                ...
                genre: String(row.genre ?? row.Genre ?? row.sexe ?? row.Sexe ?? ''),
                telephone: String(row.telephone ?? row.Telephone ?? row.Téléphone ?? ''),
            }));
            ...
            mapped.forEach((row, idx) => {
                ...
                if (!row.genre || !['M', 'F'].includes(row.genre.toUpperCase())) issues.push("'genre' invalide (utilisez M ou F)");
                if (issues.length > 0) {
    ...
    const handleImport = useCallback(async () => {
        ...
        try {
            ...
            // Map backend response to ImportResult
            const data = res.data;
            setResult({
                imported: data.count || 0,
                skipped: data.errors || 0,
                errors: [], // Advanced line-by-line backend errors aren't currently returned by this endpoint
            });
            ...
            setParseErrors([{
                line: 0,
                message: err?.response?.data?.message || err?.response?.data?.error?.message || "Erreur lors de l'import",
            }]);
        } finally {
            setIsUploading(false);
        }
    ...
    const downloadTemplate = useCallback(async () => {
        try {
            const res = await api.get('/teachers/import-template', {
                responseType: 'blob',
            });
            const url = URL.createObjectURL(res.data);
            const a = document.createElement('a');
            a.href = url;
            a.download = 'modele-import-enseignants.xlsx';
            a.click();
            URL.revokeObjectURL(url);
        } catch {
            // Fallback if the endpoint does not exist
            window.location.href = '/templates/import_teachers_template.xlsx';
        }
    }, []);
                                ...
                                <table className="w-full text-xs">
                                    <thead>
                                        <tr className="bg-neutral-100 text-neutral-600">
                                            ...
                                            <th className="px-2 py-2 text-left font-semibold">Téléphone</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-neutral-100">
                                        {previewRows.map((row, i) => (
                                            <tr key={i} className="hover:bg-neutral-50">
                                                ...
                                                <td className="px-2 py-1.5 uppercase">{row.genre}</td>
                                                <td className="px-2 py-1.5 font-mono">{row.telephone}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        <div className="space-y-5 text-center">
                            ...
                            <div>
                                <p className="text-lg font-semibold text-neutral-900">
                                    Importer {totalRows - parseErrors.length} enseignant{totalRows - parseErrors.length > 1 ? 's' : ''} ?
                                </p>
                                {parseErrors.length > 0 && (
                                ...
                                <div className="flex items-center justify-center gap-4 mt-3">
                                    ...
                                    {result.skipped > 0 && (
                                        <div className="text-center">
                                            ...
                                            <p className="text-xs text-neutral-500">ignorés / erreurs</p>
                                        </div>
                                    )}
                                </div>
                            </div>
