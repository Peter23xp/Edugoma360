...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# PROMPT 8 / 8 — SCR-017 : PALMARÈS DE CLASSE
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

```
CONTEXTE
--------
Écran : SCR-017 - Palmarès de Classe
Route : /palmares/:classId/:termId
Rôle minimum : PRÉFET
Prérequis : SCR-015 (délibération validée)
...
OBJECTIF
--------
Afficher et exporter le classement complet d'une classe.
Document officiel affiché publiquement (tableau d'honneur).


FICHIERS À CRÉER
-----------------
1. packages/client/src/pages/academic/PalmaresPage.tsx
2. packages/client/src/components/academic/PalmaresTable.tsx
3. packages/server/src/modules/reports/palmares.service.ts


UI — TABLEAU DE CLASSEMENT
----------------------------
  ┌──────────────────────────────────────────────────────────┐
  │ PALMARÈS — 4ème Scientifique A — Trimestre 2            │
  │ Année scolaire 2024-2025                                 │
  ├──────────────────────────────────────────────────────────┤
  │ [Export PDF] [Export Excel] [Imprimer]                   │
  ├──────────────────────────────────────────────────────────┤
  │ Rang │ Nom Complet           │ Moyenne │ Points │ Mention │
  ├──────┼───────────────────────┼─────────┼────────┼─────────┤
  │  1   │ BAHATI Marie-Claire   │ 15.4    │ 368    │ 🥇 Dist │
  │  2   │ DUSABE Alice          │ 15.1    │ 361    │ 🥈 Dist │
  │  3   │ AMISI Jean-Baptiste   │ 14.2    │ 340    │ 🥉 Admis│
  │  4   │ EKOKO Serge           │ 13.8    │ 331    │    Admis│
  │  5   │ FURAHA Yvette         │ 13.1    │ 314    │    Admis│
  │ ...                                                      │
  │ 28   │ CIZA Pierre           │  9.5    │ 228    │ ⚠️ Ajourn│
  │ 29   │ NGABO Paul            │  8.2    │ 197    │ ❌ Refusé│
  └──────────────────────────────────────────────────────────┘
  
  Résumé :
  • Total élèves : 35
  • Taux de réussite : 80% (28/35)
  • Moyenne de classe : 12.4/20
  • Meilleure moyenne : 15.4/20
  • Plus faible moyenne : 8.2/20
...
- 🥈 si rang = 2
- 🥉 si rang = 3
- "Grande Dist." si moy ≥ 16
- "Distinction" si 14 ≤ moy < 16
- "Admis" si 10 ≤ moy < 14
- ⚠️ "Ajourné" si 8 ≤ moy < 10
- ❌ "Refusé" si moy < 8


EXPORT PDF — Template officiel
--------------------------------
En-tête :
```
        RÉPUBLIQUE DÉMOCRATIQUE DU CONGO
    ...
    PALMARÈS — 4ÈME SCIENTIFIQUE A
    TRIMESTRE 2 — ANNÉE SCOLAIRE 2024-2025
...
- Logo école
- Rang | Nom | Moyenne | Points | Mention
- Signature Préfet
- Cachet officiel
- Date d'édition
...
API BACKEND
------------
GET /api/reports/palmares/:classId/:termId
...
GET /api/reports/palmares/:classId/:termId/pdf
  → Génère et retourne le PDF
  Content-Type: application/pdf


SERVICE — palmares.service.ts
-------------------------------
```typescript
export async function generatePalmares(
  classId: string,
  termId: string
): Promise<PalmaresData> {
  // 1. Charger tous les résultats de délibération
  const results = await prisma.delibResult.findMany({
    where: {
      deliberation: { classId, termId }
    },
    include: {
      deliberation: { include: { class: true, term: true } },
      student: true
    },
    orderBy: { rank: 'asc' }
  })
  ...
  const classAverage = results.reduce((sum, r) => 
    sum + r.generalAverage, 0
  ) / results.length
...
AFFICHAGE PUBLIC
-----------------
Option "Publier le palmarès" :
- Génère une URL publique : /public/palmares/:token
- Accessible sans authentification
- Affiché sur un écran en salle des profs
- Exportable en poster A3 pour affichage


RÈGLES MÉTIER
--------------
1. Palmarès généré seulement après délibération validée
2. Classement par ordre décroissant de total points
3. Ex-aequo affichés avec le même rang
4. Mention automatique selon moyenne
5. Signature Préfet obligatoire sur PDF officiel


DÉFINITION DE TERMINÉ
----------------------
[ ] Tableau de classement affiché correctement
[ ] Badges mentions colorés selon décision
[ ] Résumé statistiques en bas
[ ] Export PDF format officiel
[ ] Export Excel fonctionne
[ ] Option affichage public
[ ] Signature Préfet sur PDF
```

---

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# RÉCAPITULATIF MODULE ACADÉMIQUE
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

| N° | Écran | Rôle | Complexité | Fichiers |
|----|-------|------|------------|----------|
| 010 | Gestion classes | Préfet | ⭐⭐⭐ | 7 |
| 011 | Emploi du temps | Enseignant | ⭐⭐⭐ | 5 |
| 012 | Saisie notes | Enseignant | ⭐⭐⭐⭐⭐ | 9 (+ offline) |
| 013 | Vue notes classe | Secrétaire | ⭐⭐⭐ | 3 |
| 014 | Calcul moyennes | Secrétaire | ⭐⭐⭐⭐ | 4 (+ gradeCalc) |
| 015 | Délibération | Préfet | ⭐⭐⭐⭐⭐ | 6 (workflow) |
| 016 | Bulletin PDF | Secrétaire | ⭐⭐⭐⭐ | 4 (Puppeteer) |
| 017 | Palmarès | Préfet | ⭐⭐⭐ | 3 |

**Total : 8 écrans, ~41 fichiers, module le plus complexe du système**
...
```
1. SCR-010 (Classes) → Fondation
2. SCR-011 (Emploi temps) → Organisation
3. SCR-012 (Saisie notes) → CRITIQUE avec offline
4. gradeCalc.ts → AVANT SCR-014
5. SCR-013 (Vue notes) → Vérification
6. SCR-014 (Moyennes) → Préparation délibération
7. SCR-015 (Délibération) → Décisions officielles
8. SCR-016 (Bulletins) → Documents officiels
9. SCR-017 (Palmarès) → Classement public
```
...
*EduGoma 360 — Module Académique Complet — Goma, RDC — © 2025*
